...
#: Copyright notice
COPYRIGHT = """{c} Copyright (c) 2022-{year} CNES.
{c}
        ...
        for file in files:
            ...
            if file_ext in ext:
                ...
                if not match:
                    lines = content.split("\n")
                    lines.insert(
                        pos, COPYRIGHT.format(c=comment_delimiter, year=year)
                    )
                    content = "\n".join(lines)
                else:
                    content = content.replace(
                        match.group(1),
                        COPYRIGHT.format(c=comment_delimiter, year=year),
                    )
...
def main() -> None:
    """Update copyright headers in specified directories and file types."""
    ...
    for comment_delimiter, ext, dir in [
        ...
        update_copyright(
            comment_delimiter, year, ext, os.path.join(ROOT_DIR, dir)
        )
