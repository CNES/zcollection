...
from .fs import local_fs, memory_fs, s3, s3_base, s3_fs
...
def test_insert(dask_configurable, arg, request):
    ...
    data = zcollection.load("year == 2020")
    assert data is None

    data = zcollection.load("year == 2000")
    assert data is not None
    assert data.variables["time"].shape[0] == 61

    data = zcollection.load("year == 2000 and month == 4")
    assert data is not None
    dates = data.variables["time"].values
    assert numpy.all(
        dates.astype("datetime64[M]") == numpy.datetime64("2000-04-01"))

    data = zcollection.load("year == 2000 and month == 4 and day == 15")
    assert data is not None
    dates = data.variables["time"].values
    assert numpy.all(
    ...
    data = zcollection.load(
        "year == 2000 and month == 4 and day in range(5, 25)")
    assert data is not None
...
def test_drop_partitions(dask_configurable, arg, request):
    ...
    zcollection.drop_partitions("year == 2000 and month==1")
